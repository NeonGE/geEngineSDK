...
namespace geEngineSDK {
  ...
  uint32
  ModelBuilder::findOrCreateSkinnedMeshGroup(const String& name,
                                             const SPtr<VertexDeclaration>& vertexDecl,
                                             PRIMITIVE_TOPOLOGY::E topology) {
    for (uint32 i = 0; i < cast::st<uint32>(m_meshGroups.size()); ++i) {
      const ModelBuilderMeshGroup& group = m_meshGroups[i];
    ...
    return addMeshGroup(name, true, vertexDecl, topology);
  }

  uint32
  ModelBuilder::addSkinBinding(const String& name, const Vector<Matrix4>& boneOffsets) {
    SkinBinding binding;
    binding.m_name = name;
    binding.m_boneOffsets = boneOffsets;

    m_skinBindings.push_back(binding);
    return cast::st<uint32>(m_skinBindings.size() - 1);
  }
  ...
  uint32
  ModelBuilder::appendSkinnedSubMesh(uint32 meshGroupIndex,
                                     const String& name,
                                     uint32 nodeIndex,
                                     uint32 materialIndex,
    ...
    GE_ASSERT(meshGroupIndex < m_meshGroups.size());
    GE_ASSERT(nodeIndex < m_nodes.size());
    GE_ASSERT(skinBindingIndex < m_skinBindings.size());
    ...
    subMesh.materialIndex = materialIndex;
    subMesh.faceCount = faceCount;
    subMesh.isSkinned = true;
    subMesh.skinBindingIndex = skinBindingIndex;
  ...
  ModelBuilder::build() const {
    ...
    for (uint32 meshGroupIndex = 0;
      ...
      for (const auto& builderSubMesh : group.subMeshes) {
        ...
        subMesh.m_skinBindingIndex = builderSubMesh.skinBindingIndex;
